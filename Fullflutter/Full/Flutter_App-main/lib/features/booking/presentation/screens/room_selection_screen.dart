...
class _RoomSelectionScreenState extends State<RoomSelectionScreen> {
  ...
  Future<void> _fetchBookedDates() async {
    try {
      final rooms = await _bookingRepository.getRoomsCatalogByHotel(
        widget.hotelId,
      );
      ...
      for (final booking in bookings) {
        if (!roomIds.contains(booking.roomId)) continue;
      ...
      if (!mounted) return;
      setState(() {
        _bookedDates = bookedDates.toList();
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
                ...
                child: TableCalendar(
                  ...
                  onRangeSelected: (start, end, focusedDay) {
                    if (start != null && _isBooked(start)) {
                      _showBookedMessage(
                        'Khách sạn đã kín phòng vào ngày bắt đầu.',
                      );
                      return;
                    }
                    ...
                    if (start != null && end != null) {
                      ...
                      if (occupiedDays.any(_isBooked)) {
                        _showBookedMessage(
                          'Khách sạn đã kín phòng trong khoảng thời gian này.',
                        );
                        return;
                      }
                    }
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'Không còn phòng trống cho khoảng ngày này',
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (_) => PaymentScreen(
                        roomId: room.id,
                        hotelName: widget.hotelName,
                        roomNumber: room.roomNumber ?? room.id.toString(),
