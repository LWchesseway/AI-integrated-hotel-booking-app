...
public class RoomsController : ControllerBase
{
    ...
    [HttpGet("by-hotel/available")]
    [AllowAnonymous]
    public async Task<IActionResult> GetAvailableByHotelId(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var items = await _roomService.GetAvailableByHotelIdAsync(hotelId, checkInDate, checkOutDate, cancellationToken);
        return Ok(ApiResponse<IEnumerable<RoomDetailDto>>.Ok(items));
    }

    [HttpGet("by-hotel/fully-booked-dates")]
    [AllowAnonymous]
    public async Task<IActionResult> GetFullyBookedDatesByHotelId(
        [FromQuery] int hotelId,
        ...
        CancellationToken cancellationToken = default)
    {
        if (hotelId <= 0)
        {
            return BadRequest(ApiResponse<IEnumerable<DateTime>>.Fail("hotelId must be greater than 0.", 400));
        }

        var items = await _roomService.GetFullyBookedDatesByHotelIdAsync(hotelId, fromDate, toDate, cancellationToken);
